...
\begin{itemize}
  ...
  \item \textbf{Potencia Activa ($P$):} Se calcula en formato digital promediando el producto muestra a muestra de tensión y corriente (según la norma IEEE 1459-2010) y convirtiendo el valor resultante a Watts físicos mediante factores de escala:
        ...
        \end{equation}
  \item \textbf{Potencia Aparente ($S$):} Se calcula directamente a partir del producto de los valores eficaces de tensión y corriente:
        \begin{equation}
          S = V_{\text{rms}} \cdot I_{\text{rms}}
        \end{equation}
  \item \textbf{Factor de Potencia ($FP$) y Ángulo de Fase ($\phi$):} Se calculan promediando los valores obtenidos en los ciclos anteriores para filtrar ruidos. El factor de potencia se define como $FP = P/S$. Para el desfase, se calcula inicialmente la magnitud del ángulo mediante el arcocoseno:
        \begin{equation}
          \phi_{\text{mag}} = \arccos\left(\frac{P}{S}\right)
        \end{equation}
        Para determinar el signo de dicho desfase (y por ende la polaridad del ángulo), se realiza la DFT de bin único sobre las señales digitales de tensión y corriente en la frecuencia fundamental ($k=1$, correspondiente a \qty{50}{\hertz}):
        \begin{equation}
          Q_{\text{DFT}} = \left[ 0,5 \cdot (v_{\text{im}} \cdot i_{\text{re}} - v_{\text{re}} \cdot i_{\text{im}}) \right] \cdot \text{Factor}_v \cdot \text{Factor}_i
        \end{equation}
        Si $Q_{\text{DFT}} < -\qty{5}{\text{VAr}}$, se invierte el signo del ángulo ($\phi = -\phi_{\text{mag}}$), lo que corresponde a los cuadrantes III y IV (comportamiento capacitivo). De lo contrario, se mantiene positivo (comportamiento inductivo). Finalmente, se aplican bandas muertas de seguridad: ante una potencia activa $P$ dentro del rango de $\pm\qty{5}{\watt}$ el factor de potencia ($FP$) se fuerza a cero, y ante una potencia reactiva de bin único $Q_{\text{DFT}}$ dentro del rango de $\pm\qty{5}{\text{VAr}}$, el ángulo de desfase ($\phi$) se fuerza a cero.
  \item \textbf{Potencia Reactiva ($Q$):} Se calcula a partir de la potencia aparente ($S$) y el seno del ángulo de desfase ($\phi$):
        \begin{equation}
          Q = S \cdot \sin(\phi)
        \end{equation}
        El signo de $Q$ (inductivo para $Q > 0$ y capacitivo para $Q < 0$) queda determinado por el signo de $\phi$. Si se activa la banda muerta del ángulo ($\phi = 0$), la potencia reactiva resultante se fuerza a cero.
\end{itemize}

Esta metodología híbrida para la determinación del desfasaje y la potencia reactiva responde a limitaciones matemáticas y consideraciones prácticas de diseño del sistema embebido:
\begin{itemize}
  \item \textbf{Limitación matemática del arcocoseno:} Debido a que la función coseno es par ($\cos(\phi) = \cos(-\phi)$), la función inversa $\arccos(x)$ se encuentra restringida al rango de $[0, 180^{\circ}]$ (cuadrantes I y II). Por lo tanto, no es posible determinar si la corriente está adelantada (comportamiento capacitivo, $\phi < 0$) o retrasada (comportamiento inductivo, $\phi > 0$) respecto a la tensión utilizando únicamente la relación de potencia activa y aparente. El signo de la componente fundamental $Q_{\text{DFT}}$ (calculada mediante la DFT de bin único) aporta la dirección del flujo de energía reactiva, permitiendo extender el rango del ángulo de desfase a los cuatro cuadrantes ($[-180^{\circ}, 180^{\circ}]$).
  \item \textbf{Consistencia y simplificación de la calibración:} Calcular la magnitud de la potencia reactiva a través del triángulo de potencias mediante $Q = S \cdot \sin(\phi)$ (equivalente a la magnitud de $\sqrt{S^2 - P^2}$) asegura que los cálculos se mantengan acotados por los valores eficaces globales. Además, presenta una gran ventaja en el proceso de calibración del instrumento: la magnitud de $Q$ hereda de forma directa las constantes de calibración aplicadas a $V_{\text{rms}}$, $I_{\text{rms}}$ y $P$, evitando tener que calibrar un factor de escala adicional para la magnitud de la DFT de forma aislada en el firmware.
\end{itemize}
