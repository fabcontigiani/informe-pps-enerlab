...
\begin{itemize}
  ...
  \item \textbf{Potencia Aparente ($S$):} Se calcula directamente a partir del producto de los valores eficaces de tensión y corriente:
        ...
        \end{equation}
  \item \textbf{Factor de Potencia ($FP$) y Ángulo de Fase ($\phi$):} El factor de potencia se define como $FP = P/S$. Para el desfase, se calcula inicialmente la magnitud del ángulo mediante el arcocoseno:
        \begin{equation}
        ...
        \begin{equation}
          Q_{\text{DFT}} = 0,5 \cdot (v_{\text{im}} \cdot i_{\text{re}} - v_{\text{re}} \cdot i_{\text{im}}) \cdot \text{Factor}_v \cdot \text{Factor}_i
        \end{equation}
...
Esta metodología híbrida para la determinación del desfasaje responde a la limitación matemática de la función arcocoseno. Debido a que la función coseno es par ($\cos(\phi) = \cos(-\phi)$), la función inversa $\arccos(x)$ se encuentra restringida al rango de $[0, 180^{\circ}]$ (cuadrantes I y II). Por lo tanto, no es posible determinar si la corriente está adelantada (comportamiento capacitivo, $\phi < 0$) o retrasada (comportamiento inductivo, $\phi > 0$) respecto a la tensión utilizando únicamente la relación de potencia activa y aparente. El signo de la componente fundamental $Q_{\text{DFT}}$ (calculada mediante la DFT de bin único) aporta la dirección del flujo de energía reactiva, permitiendo extender el rango del ángulo de desfase a los cuatro cuadrantes.

A continuación se detallan las características y funciones del gateway de comunicaciones y seguridad, basado en el microcontrolador ESP32-C3.
