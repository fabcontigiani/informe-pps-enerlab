...
La implementación del control dinámico de ganancia permite mantener linealidad y precisión en potencias inferiores a los \qty{500}{\watt}, conservando la capacidad de registrar valores máximos de hasta \qty{17.5}{\kilo\watt}. Esto confiere al instrumento una característica distintiva frente a soluciones comerciales consolidadas \cite{shelly_pro_3em}, las cuales implementan zonas muertas (\textit{deadbands}) que omiten el registro bajo ciertos umbrales de corriente, resultando en pérdidas de información sobre el consumo de la instalación.



\subsubsection{Etapa de Comunicaciones y Gateway}

La etapa de comunicaciones se basa en el módulo ESP32-C3, el cual actúa como pasarela de datos (\textit{gateway}) entre el núcleo de medición local y la infraestructura en la nube. Desde el punto de vista del hardware, el ESP32-C3 recibe alimentación de la línea de \qty{5}{\volt} provista por la fuente HLK-5M05 y establece comunicación con el STM32F103 a través de un canal serie UART bidireccional. Esta interfaz física permite la transferencia de tramas JSON estructuradas con los valores de medición calculados. El diseño del circuito impreso prevé la orientación del módulo para evitar obstrucciones en la antena de RF integrada, asegurando la calidad de la señal Wi-Fi.
...
\begin{figure}[htbp]
  ...
  \caption{Diagrama esquemático general del sistema de medición y gateway.}
...
\begin{itemize}
  \item \textbf{Escenario 1: Señal Senoidal Pura.} Se evaluó el desempeño con corrientes de \qty{10}{\ampere} y \qty{20}{\ampere}, manteniendo un factor de potencia unitario ($PF=1$) y un factor de cresta de 3 ($CF=3$). Se probaron configuraciones con la referencia del ADC centrada en \qty{1.65}{\volt} y referenciada a masa (GND).
...
En cuanto a la gestión de los servicios, se evaluaron las opciones \textit{Fully Managed} (servicios en la nube de terceros) frente a \textit{Self-Managed} (servidores propios). Para este prototipo, se optó por una arquitectura Centralizada y Self-Managed, desplegando microservicios mediante contenedores Docker en un VPS. Esta elección permite un control total sobre la soberanía de los datos y una reducción de costos operativos durante la etapa de desarrollo.
...
La arquitectura general del sistema en la nube y los flujos de datos entre sus componentes principales se presentan en la figura~\ref{fig:arquitectura_cloud}. Esta topología se estructura en torno a tres entornos interconectados:
...
\begin{itemize}
  ...
  \item \textbf{TimescaleDB:} Base de datos de series temporales, basada en PostgreSQL, donde se almacenan de forma persistente y eficiente los registros de medición.
  \item \textbf{Node-RED:} Motor de flujos que actúa como \textit{middleware} para suscribirse a los tópicos MQTT, procesar las tramas JSON entrantes e insertarlas en la base de datos.
  \item \textbf{Grafana:} Plataforma web interactiva utilizada para la visualización de datos en tiempo real, generación de tableros de control e implementaciones de control de acceso.
...
\end{itemize}
El panel de control se ilustra en la figura~\ref{fig:db_schema} en la que se demuestra una consulta SQL sobre la tabla de datos.
