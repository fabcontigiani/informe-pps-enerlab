...
\textbf{Node-RED} actúa como capa de integración lógica del sistema (\textit{middleware}). Entre sus responsabilidades se incluyen:
...
\begin{description}[leftmargin=0pt]
  \item[Metodología de ensayo:] Se utilizó una fuente de alimentación variable capaz de simular cargas resistivas, inductivas y capacitivas, permitiendo generar corrientes de hasta \SI{50}{\ampere} RMS con factores de potencia ajustables. Para cada punto de ensayo, se registraron simultáneamente las mediciones del prototipo y del instrumento de referencia, para luego calcular el error relativo porcentual y realizar un análisis de linealidad mediante ajuste de regresión lineal.
  
  Para asegurar una evaluación exhaustiva, la validación se planteó como una matriz cruzada combinando distintos niveles de corriente y variaciones en el factor de potencia. Específicamente, el sistema fue evaluado bajo corrientes de \qty{0.5}{\ampere}, \qty{1}{\ampere}, \qty{5}{\ampere}, \qty{10}{\ampere}, \qty{20}{\ampere}, \qty{35}{\ampere} y \qty{50}{\ampere} RMS. Para corrientes iguales o superiores a \qty{5}{\ampere}, se realizaron mediciones tanto en modo de consumo —con desfase nulo (\qty{0}{\degree}) y desplazamientos angulares de $\pm\qty{15}{\degree}$, $\pm\qty{30}{\degree}$ y $\pm\qty{45}{\degree}$ (abarcando corrientes tanto en adelanto como en atraso)— como en modo de inyección de energía a la red, aplicando un desfase de \qty{180}{\degree} (midiendo a \qty{180}{\degree} y desplazamientos de $\qty{180}{\degree} \pm \qty{15}{\degree}$, $\qty{180}{\degree} \pm \qty{30}{\degree}$ y $\qty{180}{\degree} \pm \qty{45}{\degree}$). Por el contrario, en los escenarios de baja carga (corrientes menores a \qty{5}{\ampere}), debido a que la medición del ángulo presenta mayores fluctuaciones, los ensayos se limitaron a los escenarios principales de consumo e inyección pura (\qty{0}{\degree} y \qty{180}{\degree}) y a los desplazamientos de $\pm\qty{30}{\degree}$ respecto de los mismos. Este enfoque multivariable, documentado mediante suficientes ciclos por punto para obtener medias y desviaciones estándar robustas, permite verificar empíricamente que el cálculo de potencia activa y el bloque de compensación de fase en el firmware mantienen su exactitud independientemente del tipo de instalación (inductiva, puramente resistiva o capacitiva) y del sentido del flujo de energía, además de someter a prueba la respuesta de la etapa de ganancia automática (\textit{auto-ranging}) en corrientes bajas.

  \item[Resultados de exactitud:] En la figura~\ref{fig:exactitud_potencia_placeholder} se presenta el comportamiento del error relativo de potencia activa en función del nivel de carga. De manera complementaria, la figura~\ref{fig:exactitud_rms_placeholder} resume el error relativo para tensión y corriente RMS. En ambos casos se observa que el error se mantiene acotado dentro de una banda compatible con los objetivos de diseño del prototipo para monitoreo energético distribuido.
...
\begin{figure}[H]
  ...
  \fbox{%
    \begin{minipage}{0.9\textwidth}
      \centering
      \vspace{3cm}
      [Placeholder: Gráfico de error relativo de potencia activa vs. nivel de carga]
      \vspace{3cm}
    \end{minipage}%
  }
  \caption{Error relativo de potencia activa para distintos niveles de carga.}
  \label{fig:exactitud_potencia_placeholder}
\end{figure}
...
\begin{figure}[H]
  ...
  \fbox{%
    \begin{minipage}{0.9\textwidth}
      \centering
      \vspace{3cm}
      [Placeholder: Gráfico de error relativo de tensión y corriente RMS]
      \vspace{3cm}
    \end{minipage}%
  }
  \caption{Error relativo de tensión y corriente RMS respecto del instrumento de referencia.}
  \label{fig:exactitud_rms_placeholder}
\end{figure}

  \item[Resultados de linealidad:] La linealidad del canal de medición se evaluó graficando el valor reportado por el prototipo frente al valor de referencia para cada punto de ensayo. La figura~\ref{fig:linealidad_placeholder} muestra la nube de puntos con su recta de ajuste. El comportamiento observado evidencia una relación prácticamente lineal en el rango analizado, con desviaciones pequeñas y sin tendencias marcadas en los residuales.
...
\begin{figure}[H]
  ...
  \fbox{%
    \begin{minipage}{0.9\textwidth}
      \centering
      \vspace{3cm}
      [Placeholder: Gráfico de linealidad (valor medido vs. valor de referencia) con ajuste lineal]
      \vspace{3cm}
    \end{minipage}%
  }
  \caption{Evaluación de linealidad del sistema mediante comparación directa contra referencia.}
  \label{fig:linealidad_placeholder}
\end{figure}

\end{description}

En síntesis, los ensayos de exactitud y linealidad validan que la arquitectura de medición propuesta es adecuada para su aplicación en monitoreo fotovoltaico trifásico bidireccional. La combinación de acondicionamiento analógico, muestreo sincronizado y compensación digital de fase permitió sostener una respuesta estable y consistente en todo el rango de operación ensayado.
